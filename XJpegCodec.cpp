...
//-----------------------------------------------------------------------------
bool XJpegCodec::Decompress(byte* jpeg, uint32 size_in, byte* out, uint32 size_out, 
														byte* tables, uint32 tablesize)
{
	// Lecture JPEG
	...
	for (uint32 i = 0; i < cinfo.output_height; i++)
...
// Decompression brute
//-----------------------------------------------------------------------------
bool XJpegCodec::DecompressRaw(byte* jpeg, uint32 size_in, byte* out, uint32 size_out,
															 byte* tables, uint32 tablesize)
{
	// Lecture JPEG
	struct jpeg_decompress_struct cinfo;
	...
	// Raw data
	uint32 max_lines = cinfo.max_v_samp_factor * DCTSIZE;
	...
	byte* raw_buf = new byte[max_lines * cinfo.max_h_samp_factor * cinfo.output_width];
	for (uint32 i = 0; i < max_lines * cinfo.max_h_samp_factor; i++)
	...
	while (cinfo.output_scanline < cinfo.output_height) {
		...
		for (uint32 i = 0; i < num_scanlines; i++) {
			for (uint32 j = 0; j < cinfo.output_width; j++) {
