...
XLzwString::XLzwString(int n, byte* str)
{
  m_nLength = n;
  m_nAlloc = n;
  m_String = new byte[m_nAlloc];
...
void XLzwCodec::SetDataIO(byte* lzw, byte* out, uint32 size)
{
  m_lzw = lzw;
  m_out = out;
  m_outpos = out;
...
void XLzwCodec::AddStringToTable(uint32 oldcode, uint32 code)
{
...
{
  uint32 code, oldcode = 0, ClearCode = 256, EoiCode = 257;
